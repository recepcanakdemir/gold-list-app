...
export default function DashboardScreen() {
  ...
  // ✨ LOCAL STATE: For instant dashboard updates
  const {
    addWordsOptimistic,
  ...
  useEffect(() => {
    ...
    const handleWordsAdded = (data: { notebookId: string; wordCount: number }) => {
      if (__DEV__) console.log('📊 Dashboard: wordsAdded - instant local + cache updates', data)
      
      // ✨ INSTANT: Update local state first for immediate UI feedback
      addWordsOptimistic(data.notebookId, data.wordCount)
      addActivityOptimistic() // Predict streak increase
      
      // ✨ SMART CACHE UPDATES: Update existing data instead of invalidating
      const today = getCurrentDate().toISOString().split('T')[0]
      
      // 1. Update today's progress immediately
      queryClient.setQueryData(['today-progress', profile?.id, today], (old: any) => {
        if (!old) return old
        return {
          ...old,
          wordsAdded: (old.wordsAdded || 0) + data.wordCount,
          completed: (old.wordsAdded || 0) + data.wordCount >= (old.goal || 20)
        }
      })
      
      // 2. Update weekly progress (add to today's entry)
      queryClient.setQueryData(['weekly-progress', profile?.id, today], (old: any) => {
        if (!old || !Array.isArray(old)) return old
        const todayIndex = old.findIndex(day => day.day === new Date().toLocaleDateString('en-US', { weekday: 'short' }))
        if (todayIndex >= 0) {
          const updated = [...old]
          updated[todayIndex] = {
            ...updated[todayIndex],
            wordsAdded: (updated[todayIndex].wordsAdded || 0) + data.wordCount,
            completed: (updated[todayIndex].wordsAdded || 0) + data.wordCount >= 20
          }
          return updated
        }
        return old
      ...
      // 3. Update daily progress (heatmap) - add to today's entry
      queryClient.setQueryData(['daily-progress', profile?.id, currentSimulatedDay, 175], (old: any) => {
        if (!old || !Array.isArray(old)) return old
        const todayEntry = old.find(day => day.date === today)
        if (todayEntry) {
          return old.map(day => day.date === today ? 
            { ...day, wordsAdded: (day.wordsAdded || 0) + data.wordCount } : day
          )
        }
        return old
      })
      
      // 4. Only invalidate stats that require complex recalculation
    ...
    const handleReviewsCompleted = (data: { notebookId: string; reviewCount: number }) => {
      if (__DEV__) console.log('📊 Dashboard: reviewsCompleted - instant local + cache updates', data)
      ...
      // 1. Update daily progress (heatmap) - add reviews to today's entry
      queryClient.setQueryData(['daily-progress', profile?.id, currentSimulatedDay, 175], (old: any) => {
        if (!old || !Array.isArray(old)) return old
        const todayEntry = old.find(day => day.date === today)
        if (todayEntry) {
          return old.map(day => day.date === today ? 
            { ...day, wordsReviewed: (day.wordsReviewed || 0) + data.reviewCount } : day
          )
        }
        return old
      })
      
      // 2. Only invalidate complex stats that require recalculation
      queryClient.invalidateQueries({ queryKey: ['total-stats'] })
      queryClient.invalidateQueries({ queryKey: ['weekly-progress'] })
    }
        ...
        <View style={styles.section1Card}>
          ...
          <View style={styles.streakMasteredRow}>
            <View style={styles.streakMasteredItem}>
              ...
              <Text style={styles.streakMasteredValue}>{getPredictedStreak(profile?.streak_count || 0)}</Text>
