...
export default function HomeScreen() {
  ...
  const getNotebookStatus = (notebook: NotebookWithStats) => {
    ...
    // ✨ INSTANT: Get predicted state using local state hook for immediate UI feedback
    ...
    // Get instant predictions from local state
    const predictedButtonState = getPredictedButtonState(notebook, dbProgress, dbHasReviews)
    
    if (__DEV__) {
      console.log(`🎯 Button state prediction for ${notebook.title}:`, predictedButtonState)
    }
  ...
  const getUserStats = () => {
    // ✨ INSTANT: Use predicted streak for immediate UI feedback
    const dbStreakDays = profile?.streak_count || 0
    const predictedStreakDays = getPredictedStreak(dbStreakDays)
    ...
    return {
      ...
      streakDays: predictedStreakDays,
      successRate,
