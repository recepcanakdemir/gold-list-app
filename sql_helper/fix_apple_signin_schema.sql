-- =============================================
-- FIX APPLE SIGN-IN DATABASE SCHEMA ISSUE
-- =============================================
-- This script fixes the database schema type mismatch that was causing
-- Apple Sign-In profile creation to fail with:
-- "column last_activity_date is of type date but expression is of type text"
--
-- The trigger was working perfectly and extracting Apple data correctly,
-- but failing on the database insert due to CURRENT_DATE::TEXT vs DATE type mismatch
-- 
-- Run this in Supabase SQL Editor

-- =============================================
-- CREATE OR REPLACE THE FIXED TRIGGER FUNCTION
-- =============================================
...
BEGIN
    ...
    -- CRITICAL FIX: Changed CURRENT_DATE::TEXT to CURRENT_DATE (DATE type instead of TEXT type)
    INSERT INTO public.profiles (
        ...
        0,
        CASE 
            WHEN auth_provider != 'email' THEN CURRENT_DATE  -- FIXED: Removed ::TEXT cast
            ELSE NULL
        END,
...
-- Verify the trigger still exists and points to our function
SELECT 
    trigger_name,
    event_manipulation,
    action_statement,
...
-- Test logging to confirm it works
DO $$
BEGIN
    RAISE LOG 'SCHEMA FIX TEST: Database schema fix applied - timestamp: %', NOW();
    RAISE LOG 'SCHEMA FIX TEST: last_activity_date will now use CURRENT_DATE (DATE type) instead of CURRENT_DATE::TEXT';
END $$;
...
SELECT 'SCHEMA FIX COMPLETE - Ready for testing' as status;
SELECT 'Next steps:' as instruction;
SELECT '1. Delete existing Apple test users from auth.users table' as step1;
SELECT '2. Create fresh Apple Sign-In user' as step2;  
SELECT '3. Check postgres logs for successful profile creation' as step3;
SELECT '4. Verify profile has correct auth_provider, email, and apple_user_id' as step4;